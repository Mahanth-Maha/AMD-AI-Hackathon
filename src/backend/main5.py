...
def blood_relationships_batch_prompt(topic: str, seed_context: str) -> str:
    return f"""
...
These puzzles must involve blood relationships and **include enough context within the question** to be solvable. Do **not** rely on external knowledge or assumptions — the question itself must provide everything needed to deduce the answer.
...
- Each puzzle must be simple and self-contained.
...
- Use or build around this context if needed:
  <CONTEXT>
  {seed_context}
  </CONTEXT>
...
def get_textfiles_and_extract_delta():
    ...
    # Extract the highest number from filenames like "br_dataset_1.json"
    numbers = []
    for filename in txt_files:
        match = re.search(r'br_dataset_(\d+)\.json', filename)
        if match:
...
for i in tqdm(range(N), desc="Generating batches"):
    ...
    prompt = blood_relationships_batch_prompt(topic, context)
    result = call_vllm_chat(prompt)

    output_filename = f"br_dataset_{i+ 1 + delta}.json"
    if result:
