...
def seatting_arrangement_batch_prompt(topic: str, seed_context: str) -> str:
    return f"""
...
These puzzles must involve seating arrangements or family trees and **include enough context within the question** to be solvable. Do **not** rely on external knowledge or assumptions — the question itself must provide everything needed to deduce the answer.
...
N = 5  # Number of batches to generate
...
for i in tqdm(range(N), desc="Generating batches"):
    ...
    prompt = seatting_arrangement_batch_prompt(topic, context)
    result = call_vllm_chat(prompt)

    output_filename = f"sa_dataset_{i+1}.json"
    if result:
