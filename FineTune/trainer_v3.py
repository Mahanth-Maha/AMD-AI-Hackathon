...
class AptitudeTrainer:
    ...
    def __init__(self, args):
        ...
        Your task is to answer multiple-choice questions (MCQs) on the topic of "Truth-teller and Liar Problems", "Seating Arrangements (Linear, Circular)", and "Puzzles involving generations and family tree logic".
        ...
        Make sure to follow the JSON format strictly i.e. start with {self.reasoning_start} and end with {self.reasoning_end} for reasoning,
        ...
        Ensure the answer is clear, concise, and well-structured.
        """
        ...
        You are an expert in logical reasoning and complex problem-solving.
        Your task is to generate multiple-choice questions (MCQs) on the topic of "Truth-teller and Liar Problems", "Seating Arrangements (Linear, Circular)", and "Puzzles involving generations and family tree logic".
        Make sure to follow the JSON format strictly.
        Each question should have a clear question text, four options (A, B, C, D), and the correct answer.
        The question should be challenging and require logical reasoning to solve.
        Ensure the question is self-contained and does not require external knowledge.
        The answer should be a single letter (A, B, C, or D) representing the correct option.
        ...
        Ensure the question is clear, concise, and well-structured.
        """
        # Setup environment
    ...
    def _setup_environment(self):
        ...
        port_random = random.randint(29500, 30000)
        os.environ.setdefault("MASTER_PORT", str(port_random))
            ...
            for idx, question_obj in enumerate(question_objects, 1):
                try:
                    ...
                    if "question" in question_obj:
                        ...
                        topic = question_obj.get("topic", random_topic)
    ...
    def _format_grpo_dataset(self, raw_items: List[Dict[str, str]]) -> Dataset:
        """Format raw items for GRPO training using tagged JSON fields."""
        formatted_items = []

        for item in raw_items:
            topic = item.get("topic", "Unknown Topic")
            question = item['original_question'] if 'original_question' in item else item['question']
            answer_letter = item['answer']
            reasoning = item.get("reasoning", "No explanation provided.")
            choices = item.get("choices", ["", "", "", ""])

            # Tagged prompt format
            tagged_prompt = (
                f"{self.topic_start}{topic}{self.topic_end}\n"
                f"{self.question_start}{question}{self.question_end}\n"
                f"{self.choices_A_start}{choices[0]}{self.choices_A_end}\n"
                f"{self.choices_B_start}{choices[1]}{self.choices_B_end}\n"
                f"{self.choices_C_start}{choices[2]}{self.choices_C_end}\n"
                f"{self.choices_D_start}{choices[3]}{self.choices_D_end}\n"
                f"{self.answer_start}{answer_letter}{self.answer_end}\n"
                f"{self.explanation_start}{reasoning}{self.explanation_end}"
            )
...
if __name__ == "__main__":
    """
    APTITUDE UNIFIED TRAINER
    ================================

    This script provides a unified interface for training aptitude reasoning models
