...
class QuestioningAgent(object):
    ...
    def __init__(self, model_name: str = "/jupyter-tutorial/hf_models/Qwen3-4B", **kwargs):
        self.agent = QAgent(model_name=model_name, **kwargs)
    ...
    def build_prompt(self, topic: str, wadvsys: bool = True, wicl: bool = True, inc_samples: List[Dict[str, str]] | None = None) -> Tuple[str, str]:
        """Builds a state-of-the-art prompt for question generation"""

        if wadvsys:
            sys_prompt = (
                "You are an expert MCQ creator with deep experience in competitive exam questions. "
                "You must generate HIGH-QUALITY, HARD questions in JSON format ONLY, without showing any thinking steps or solution trace."
            )
        else:
            sys_prompt = "You are a senior question setter for logic-based exams."
        ...
        tmpl = (
            f"You must now generate a **valid JSON object** with the following required fields ONLY:\n"
            f"- \"topic\": a short title for the logic topic.\n"
            f"- \"question\": the full text of the MCQ.\n"
            f"- \"choices\": a list of EXACTLY 4 strings starting with 'A)', 'B)', 'C)', 'D)'.\n"
            f"- \"answer\": a SINGLE uppercase letter: A, B, C, or D. (Only the correct option letter.)\n"
            f"- \"explanation\": a short, under-100-words justification of why the answer is correct.\n\n"

            f"🛑 Do NOT include backticks (`), codeblocks (```json), or extra commentary.\n"
            f"🛑 DO NOT prefix the answer like 'A) A)'. Just use one 'A)' per choice.\n\n"

            f"=== Your Task ===\n"
            f"Create a very difficult multiple-choice question strictly on:\n"
            f"🧠 Topic: {topic}\n"
            f"👉 Option {correct_option}) must be the ONLY correct answer.\n"
            f"👉 Options {distractors} must be plausible yet incorrect distractors.\n"
            f"👉 Ensure the explanation justifies why ONLY {correct_option} is correct.\n\n"
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--model_name", type=str, help="Path to the pre-trained model.", default="models/question_model")
    ...
    agent = QuestioningAgent(model_name=args.model_name) 
    ...
    for q in question:
        try:
            ...
            prompt = (
                "You are an expert JSON extractor trained to extract structured logical reasoning data from noisy or mixed-format text.\n\n"
                "⚠️ STRICTLY follow these instructions:\n"
                "1. Extract and return ONLY the following five keys:\n"
                "   - \"topic\"\n"
                "   - \"question\"\n"
                "   - \"choices\" (a list of exactly 4 strings, each starting with 'A)', 'B)', 'C)', or 'D)')\n"
                "   - \"answer\" (just a single capital letter: A, B, C, or D)\n"
                "   - \"explanation\" (brief explanation, ≤100 words)\n\n"
                "2. DO NOT include any markdown syntax or code block formatting (e.g., no ```json, no ```)\n"
                "3. Remove ALL irrelevant text, comments, metadata, or formatting artifacts.\n"
                "4. Ensure that the output is a **valid, parsable JSON object** matching the format exactly.\n"
                "5. You MUST discard redundant label prefixes (e.g., 'A) A)' → 'A)') and normalize all choice labels.\n"
                "6. Your output must be compact, correct, and in the following format:\n\n"
                "7. Don't generate extra double quotes or escape characters which is not mentioned.\n\n"
                "8. make sure the json has starting and ending curly braces.\n\n"
                'String:\n'
                '{}\n\n'
            ...
            # remove choices like "A) A)" or "B) B)" etc.
            # q = q.replace("A) A)", "A)").replace("B) B)", "B)").replace("C) C)", "C)").replace("D) D)", "D)")
