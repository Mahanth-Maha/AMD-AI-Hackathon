...
class AnsweringAgent(object):
    ...
    def __init__(self, select_prompt1: bool = True, model_name: str = "/jupyter-tutorial/hf_models/Qwen3-4B", **kwargs):
        self.agent = AAgent(model_name=model_name, **kwargs)
    ...
    def build_prompt(self, question_data: Dict[str, str | Any]) -> Tuple[str, str]:
        """Generate an answer to the given MCQ question with strict JSON formatting"""
        
        sys_prompt = (
            "You are an expert answer agent specializing in solving multiple-choice questions (MCQs) for logic, puzzles, and aptitude.\n"
            "You MUST think step-by-step, eliminate incorrect options, and select ONLY the correct one.\n"
            "You MUST return only a valid JSON object containing:\n"
            "1. 'answer': ONE letter from [\"A\", \"B\", \"C\", \"D\"] — nothing else, no labels like 'A)', 'Option A', etc.\n"
            "2. 'reasoning': Brief explanation of WHY the chosen answer is correct (under 100 words).\n"
            "❌ DO NOT include markdown, backticks, extra text, or commentary.\n"
            "❌ DO NOT repeat the question or options.\n"
            "✅ The output must be parsable JSON (with curly braces, quotes, and commas exactly as shown).\n"
        )

        tmpl = (
            "Now solve this question and return only the required JSON:\n\n"
            "Question:\n{question}\n\n"
            "Choices:\n{choices}\n\n"
            "STRICT OUTPUT FORMAT (JSON only):\n"
            "{{\n"
            '  "answer": "A",\n'
            '  "reasoning": "Explain briefly why option A is correct."\n'
            "}}"
        )
...
if __name__ == "__main__":
    ...
    agent = AnsweringAgent(select_prompt1=SELECT_PROMPT1, model_name=args.model_name)
    ...
    for idx, (q, a) in enumerate(zip(sample_questions, answer)):
        ...
        try:
            ...
            if all(k in a for k in ['answer', 'reasoning']):
                ...
                if len(a['answer']) != 1:
                    prompt = (
                    "You are a precision extractor designed to return only the answer and its justification from a multiple-choice question context.\n\n"
                    
                    "⚠️ STRICT INSTRUCTIONS:\n"
                    "1. Extract and return ONLY the following two keys:\n"
                    "   • \"answer\": MUST be a single uppercase letter: A, B, C, or D (no extra text or punctuation).\n"
                    "   • \"reasoning\": A concise explanation (≤ 100 words) of why the selected option is correct.\n\n"
                    "2. DO NOT include:\n"
                    "   • Any markdown (e.g., ```json or backticks)\n"
                    "   • Any extra keys or metadata\n"
                    "   • Any duplicated labels like 'A) A)' (normalize to 'A)')\n"
                    "   • Any trailing commas or unescaped characters\n\n"
                    "3. Your output must be a **valid, well-formed, single-line JSON object** that strictly matches the schema shown below.\n"
                    "4. DO NOT explain or comment on your output. Just return the JSON.\n\n"
                    ...
                    '{{\n'
                    '  "answer": "A",\n'
                    '  "reasoning": "Explain why A is correct in under 100 words."\n'
                    '}}\n'
                    '\n'
                )
